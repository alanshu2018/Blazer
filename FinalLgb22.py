...
valid_file="../newdata/val_v22.csv"
test_file ="../newdata/test_v22.csv"
...
def lgb_modelfit_nocv(bst, params, dtrain, dvalid, predictors, target='target', objective='binary', metrics='auc',
                      feval=None, early_stopping_rounds=50, num_boost_round=900, verbose_eval=10, categorical_features=None,do_free=True):
    lgb_params = {
        'boosting_type': 'gbdt',
        'objective': objective,
        'metric':metrics,
...
df = pd.read_csv(train_file,engine='c',usecols=predictors+['click_id'],dtype=dtypes)
total = len(df)
val_size = 30000000
train_df =df[:(total-val_size)]
val_df = df[(total-val_size):]

print("train.len={}".format(total-val_size))
print("valid.len={}".format(val_size))
