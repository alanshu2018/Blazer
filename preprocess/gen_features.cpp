...
public:
    ...
    Counter calculate_mean_and_result_values_for_stat(map<string,Counter> &stat) {
        ...
        result_s << "," << counter.clk << "," << counter.down;
        ...
        for (int i = 0 ; i < 6; i++){
            counter.hour_6_cnt[i] /= total_num;
            result_s << "," << counter.hour_6_cnt[i];
        }
        for (int i = 0 ; i < 4; i++){
            counter.min_4_cnt[i] /= total_num;
    ...
    void calculate_mean_values(){
        ...
        mean_ip_counter = calculate_mean_and_result_values_for_stat(ip_counter);
        mean_app_counter = calculate_mean_and_result_values_for_stat(app_counter);
        mean_os_counter = calculate_mean_and_result_values_for_stat(os_counter);
        mean_device_counter = calculate_mean_and_result_values_for_stat(device_counter);
        mean_channel_counter = calculate_mean_and_result_values_for_stat(channel_counter);
        mean_ip_app_counter = calculate_mean_and_result_values_for_stat(ip_app_counter);
        mean_ip_os_counter = calculate_mean_and_result_values_for_stat(ip_os_counter);
        mean_ip_device_counter = calculate_mean_and_result_values_for_stat(ip_device_counter);
        mean_ip_channel_counter = calculate_mean_and_result_values_for_stat(ip_channel_counter);
        mean_app_os_counter = calculate_mean_and_result_values_for_stat(app_os_counter);
        mean_app_device_counter = calculate_mean_and_result_values_for_stat(app_device_counter);
        mean_app_channel_counter = calculate_mean_and_result_values_for_stat(app_channel_counter);
        mean_os_device_counter = calculate_mean_and_result_values_for_stat(os_device_counter);
        mean_device_channel_counter = calculate_mean_and_result_values_for_stat(device_channel_counter);
        mean_ip_app_os_counter = calculate_mean_and_result_values_for_stat(ip_app_os_counter);
        mean_ip_app_device_counter = calculate_mean_and_result_values_for_stat(ip_app_device_counter);
        mean_ip_app_channel_counter = calculate_mean_and_result_values_for_stat(ip_app_channel_counter);
        mean_app_os_device_counter = calculate_mean_and_result_values_for_stat(app_os_device_counter);
        mean_app_device_channel_counter = calculate_mean_and_result_values_for_stat(app_device_channel_counter);
    }
    ...
    void do_gen_features_from_file(string fn,int start,int is_train) {
            ...
            while (getline(fin,line,'\n')){
                ...
                if(is_train){
                    ...
                    gen_features_for_row(row, fo, ip, app, os, device, channel);
                }else{
