    ...
    for (train_x, train_y) in train_data.get_minibatch(batch_size,epochs,do_shuffle):
        ...
        for i in range(1):
            ...
            for start in range(0, total_negative, batch_size):
                ...
                pxs = pxs[:0]
                pys = pys[:0]
                for step in range(int(num/total_positive+1)):
                    pxs.append(pos_x)
                    pys.append(pos_y)
                    if len(pxs) >= num:
                        break
        ...
        for _ in range(epochs):
            for file_obj in self.file_objs:
                ...
                for df_c in pd.read_csv(file_obj.filename,engine='c',chunksize=batch_size,sep=","):
                    ...
                    X[self.config.int_features:] = np.log(np.log(X[self.config.int_features:] + 1.0) + 1)
...
def main(debug=True):
    ...
    dev1_data_loader = DataLoader(files=[
        ("newdata/train_{}.csv".format(ver),400000),
    ])
