...
class Config(object):
    ...
    """
    feature_sizes = [364778,768,4227,1000,956,24]
    ...
    int_features = 6 #len(feature_sizes)
    float_features = 29#len(feature_names_float)
...
class DeepFMModel(Model):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("predict") as scope:
            ...
            y_embedding = tf.reshape(XV,[-1, feature_num * self.config.embed_size])
    ...
    def evaluate(self, sess, dev_data):
        ...
        start = 0
        for (dev_data_x,dev_data_y) in dev_data.get_minibatch(batch_size=100000):
    ...
    def fit(self, sess, train_writer, saver, train_data, dev_data, epochs=None):
        ...
        for i, (train_x, train_y) in enumerate(train_data.get_minibatch(epochs=epochs)):
            step += 1
            #print("Training:len={}".format(len(train_x)))
...
class DataLoader(object):
    ...
    def get_minibatch(self, batch_size=100000, epochs=1, do_shuffle=True):
        for _ in range(epochs):
            for file_obj in self.file_objs:
                total_len = file_obj.total_len
                read_len = 0
                for df_c in pd.read_csv(file_obj.filename,engine='c',chunksize=batch_size,sep=","):
                    df_c.fillna(0.0)
                    X = df_c[self.config.predictors].values
                    X[self.config.int_features:] = np.log(np.log(X[self.config.int_features:] + 1.0)+1.0)
                    y = df_c['click_id'].values
                    X = np.nan_to_num(X)
                    read_len += len(y)
...
def main(debug=True):
    ...
    train_data_loader = DataLoader(files=[
        ...
        ("newdata/train_{}.csv".format(ver),40000000),
    ])
    ...
    dev1_data_loader = DataLoader(files=[
        ("newdata/train_{}.csv".format(ver),4000000),
    ])
