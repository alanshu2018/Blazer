...
class Evaluate(Callback):
    def __init__(self, model_name, model, valid_X, valid_y):
        #self.scores = []
        self.patience = 5
        ...
        self.valid_y = np.reshape(valid_y,(-1,))

    def on_epoch_end(self, epoch, logs=None):
        y_pred = self.model.predict(self.valid_X)
        y_pred = np.reshape(y_pred,(-1,))
        score = roc_auc_score(self.valid_y,y_pred)
        print("Epoch={}, auc score={}".format(epoch, score))
        if score > self.highest: # 保存最优模型权重
            self.highest = score
            self.last_best_step = epoch
            self.model.save_weights(self.model_name)
            print("Found highest: score={},best_step={}".format(score,epoch))
        elif epoch - self.last_best_step > self.patience:
            print("Early stopping: epoch={},last_epoch={}".format(epoch, self.last_best_step))
...
class MyKerasDNNRegressor(object):
    ...
    def _build(self):
        ndim = 2 #self.config['ndim']
        X = Input(shape=(ndim,),name="X")
        #y = Input(shape=(None,),name="y")

        #X1 = BatchNormalization()(X)
        X1 = X
        input = Dropout(self.config['input_dropout'])(X1)
    ...
    def fit(self,X, y, validate=True):
        if validate:
            ...
            model_name = "mykeras_best.model"

            evaluate = Evaluate(model_name,self.model,test_X,test_y)
            ...
            # SGD learning with low learning rate
